...
class Coin(pygame.sprite.Sprite):
    ...
    def spawn(self):
        """Создаёт монету в случайном месте, где нет врагов"""
        while True:
            self.rect.center = (random.randint(30, dw - 30), random.randint(30, dh - 130))
            if not pygame.sprite.spritecollideany(self, enemies):  # Проверяем, нет ли монеты на машине
                break
...
while is_playing:
    ...
    # Движение и отображение всех спрайтов
    ...
    for coin in coins:
        ...
        if pygame.sprite.collide_rect(P1, coin):  # Проверка столкновения игрока с монетой
            coin.kill()
            coin_score += 1
            new_coin = Coin()
            coins.add(new_coin)

    # Проверка столкновения игрока с машиной
